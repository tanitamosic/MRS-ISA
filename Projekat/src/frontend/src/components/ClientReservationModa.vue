            ...
            <div class="modal-wrapper">
                <div class="modal-container" style="background-color: lightgray;">
                    <div class="modal-header"
                        style="font-size: 22px; color:lime; background-color: lightslategray; padding-left: 60px;">
                        Nova rezervacija
                    </div>
                    ...
                    <div class="modal-footer">
                        <slot name="footer">
                            <button class="form-control btn btn-primary" @click="createReservation">Kreiraj</button>
                            <button class="form-control btn btn-danger" @click="$emit('close')">Odustani</button>
                        </slot>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        createReservation: function () {
            ...
            let today = new Date();
            let actionFrom = Date.parse(this.resDateFrom);
            let actionTo = Date.parse(this.resDateTo);
